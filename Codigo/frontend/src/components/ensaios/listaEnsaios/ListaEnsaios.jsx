...
import EnsaioDetailsModal from './EnsaioDetailsModal'
...
const buildParams = (filters) => {
  const params = {}
  if (filters.clienteNome.trim()) params.clienteNome = filters.clienteNome.trim()
  if (filters.tipo) params.tipo = filters.tipo
  if (filters.status) params.status = filters.status
  if (filters.dataInicio) params.dataInicio = new Date(`${filters.dataInicio}T00:00:00`).toISOString()
  if (filters.dataFim) params.dataFim = new Date(`${filters.dataFim}T23:59:59`).toISOString()
  return params
}

const sortValue = (ensaio, key) => {
  if (key === 'dataEnsaio') return ensaio.dataEnsaio ? new Date(ensaio.dataEnsaio).getTime() : 0
  if (key === 'valorPacote' || key === 'progresso') return Number(ensaio[key] || 0)
...
export default function ListaEnsaios() {
  ...
  const paginatedEnsaios = sortedEnsaios.slice((page - 1) * pageSize, page * pageSize)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg = error?.response?.data?.erro || error?.response?.data?.message || 'Não foi possível excluir este ensaio.'
      showToast(msg, 'error')
    } finally {
  ...
  const handleStatus = async (status) => {
    if (!selected || selected.status === status) return closeModal()
    setActionLoading(true)
    try {
  ...
  const handleEdit = async (payload) => {
    ...
    try {
      ...
      const msg = error?.response?.data?.message || error?.response?.data?.error || 'Não foi possível salvar as alterações.'
      showToast(msg, 'error')
    } finally {
      setActionLoading(false)
    }
    ...
    <>
      <main className="relative z-[1] mx-auto max-w-[1200px] px-8 pb-16 pt-[88px] animate-[fadeUp_0.55s_cubic-bezier(0.22,1,0.36,1)_both] max-md:px-4">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="mb-1 text-[10.5px] uppercase tracking-[0.2em] text-white/30">Gestão</div>
            <h1 className="font-serif text-[32px] font-light tracking-[0.04em] text-white">Ensaios</h1>
            <p className="mt-0.5 text-[12px] text-white/35">{countLabel}</p>
          </div>
        ...
        <StatusTabs activeStatus={filters.status} onChange={(value) => handleFilterChange('status', value)} />
          ...
          <>
            {viewMode === 'table' ? (
              <EnsaiosTable
                ...
                sort={sort}
                onSort={handleSort}
                onView={(ensaio) => openModal('details', ensaio)}
      ...
      <EnsaioDetailsModal
        ...
        onEdit={(ensaio) => openModal('edit', ensaio)}
      />
      <EditEnsaioModal open={modal === 'edit'} ensaio={selected} loading={actionLoading} onClose={closeModal} onSave={handleEdit} />
      <StatusModal open={modal === 'status'} ensaio={selected} loading={actionLoading} onClose={closeModal} onConfirm={handleStatus} />
      <ConfirmDeleteModal open={modal === 'delete'} ensaio={selected} loading={actionLoading} onClose={closeModal} onConfirm={handleDelete} />
