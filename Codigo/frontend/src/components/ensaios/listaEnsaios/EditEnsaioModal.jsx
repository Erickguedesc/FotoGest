...
export default function EditEnsaioModal({ ensaio, open, loading, onClose, onSave }) {
  ...
  return (
    <BaseModal
      ...
      footer={(
        <>
          ...
          <button type="submit" form="edit-ensaio-form" disabled={loading} className="rounded-lg bg-[var(--gold)] px-5 py-2.5 text-[12px] font-medium tracking-[0.1em] text-[#1A1200] transition hover:bg-[var(--gold-light)] disabled:opacity-60">
    ...
    >
      {form && (
        <form id="edit-ensaio-form" onSubmit={submit} className="space-y-4">
          <div className="rounded-lg border border-white/[0.08] bg-white/[0.03] p-3 text-[13px] text-white/60">
            Cliente: <span className="text-white">{ensaio?.clienteNome}</span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label>
              ...
              <input required type="datetime-local" value={form.dataEnsaio} onChange={(event) => change('dataEnsaio', event.target.value)} className={inputClass} />
            </label>
          </div>

          <label className="block">
            <span className={labelClass}>Local</span>
            <input required value={form.local} onChange={(event) => change('local', event.target.value)} className={inputClass} placeholder="Ex: Studio Olhari, BH" />
          </label>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label>
              <span className={labelClass}>Qtd. fotos</span>
              <input required min="1" type="number" value={form.qtdFotosPacote} onChange={(event) => change('qtdFotosPacote', event.target.value)} className={inputClass} />
            </label>
            <label>
              <span className={labelClass}>Valor pacote</span>
              <input required min="0.01" step="0.01" type="number" value={form.valorPacote} onChange={(event) => change('valorPacote', event.target.value)} className={inputClass} />
            </label>
          </div>

          <label className="flex items-center gap-2 text-[13px] text-white/65">
            <input type="checkbox" checked={form.cobrarFotoExtra} onChange={(event) => change('cobrarFotoExtra', event.target.checked)} />
            Cobrar foto extra
          </label>
          ...
          {form.cobrarFotoExtra && (
            <label className="block">
              ...
              <input required min="0.01" step="0.01" type="number" value={form.valorFotoExtra} onChange={(event) => change('valorFotoExtra', event.target.value)} className={inputClass} />
            </label>
          )}

          <label className="block">
            <span className={labelClass}>Observações</span>
            <textarea maxLength="400" rows="4" value={form.observacoes} onChange={(event) => change('observacoes', event.target.value)} className={`${inputClass} resize-y`} />
          </label>
        </form>
      )}
    </BaseModal>
