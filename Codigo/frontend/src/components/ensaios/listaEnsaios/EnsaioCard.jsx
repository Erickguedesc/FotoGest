...
export default function EnsaioCard({ ensaio, onView, onEdit, onStatus, onDelete, onPreContrato }) {
  ...
  return (
    <article className="theme-card group overflow-hidden rounded-[8px] border transition hover:-translate-y-0.5 hover:border-[var(--gold-border)]">
      <button
        type="button"
        onClick={() => onView(ensaio)}
        className="relative block h-[188px] w-full overflow-hidden bg-[#151310] text-left"
      ...
      >
        {capaUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_50%_35%,#2b2520_0%,#151310_62%,#0d0d0c_100%)]">
            <span className="flex h-[86px] w-[86px] items-center justify-center rounded-full border border-white/[0.12] bg-white/[0.04] font-serif text-[25px] text-white/50">
      ...
      <div className="p-4">
        <button type="button" onClick={() => onView(ensaio)} className="block w-full text-left">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="theme-title truncate font-serif text-[22px] leading-tight">
          ...
          <div className="theme-text mt-4 grid gap-2 text-[12px]">
            ...
            <span className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate">{ensaio.local || 'Local nao informado'}</span>
            </span>
          </div>
        ...
        <div className="theme-divider mt-4 flex justify-end border-t pt-3" onClick={(event) => event.stopPropagation()}>
          <EnsaioActions
            ensaio={ensaio}
            onView={onView}
            onEdit={onEdit}
